...
float map(vec3 p){
    ...
    vec3 q2 = p * 25.0 + uTime;
    float zoom = uZoom;

    // return length( q + vec3 (cos(uTime * 0.002)) * log(length(p) + 0.6) + cos(q.y + cos(q.x + cos(q.z))) * 1.0 - zoom);
    // return length( p + vec3 (sin(uTime * 1.7))) * log(length(p) + 0.6) + cos(q.y + cos(q.z + cos(q.x))) * uChaos - zoom;
              

    // return length( sin(q2 * 0.5) + vec3 (cos(0.2 * uTime))) * log(length(q) * 0.1) + cos(p.x + cos(p.z + sin(p.y))) * uChaos * uDeform;
    // return length( cos(q2 * 2.5) + vec3 (sin(uTime * 1.2))) * log(length(q2) * 0.01) + sin(p.x + cos(p.y + cos(p.x))) * uChaos * uDeform;
    return length( sin(q2 * 0.5) + vec3 (sin(0.02 * 20.2))) * log(length(q2) * 0.01) + sin(p.y + sin(p.x * sin(p.z))) * uChaos * uDeform;
...
void main(){	
    ...
    for(int i = 0; i <= 5; i++)	{
        ...
        if (pinkBlossom) {
            ...
            vec3 color = vec3(vUv, red) - vec3( vUv, red) * f;
                    cl = cl * color + smoothstep(uLight, .8, rAmp) * .8 * color;
                        spect ? aurora -= spectral : aurora += min(rz, 1.5);
        } else {
            float f =  clamp((rz - map(position + uChaos)) * .5, + 2.1, 1. );
            vec3 color = vec3(vUv, 0.05) - vec3(vUv, 0.02) * f;
